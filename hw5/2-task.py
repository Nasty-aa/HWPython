...
def repeats(our_str):
    letters_dict = {}  # Создаем словать для букв
    new_str = ''  # Переменная для итоговой строки
    array_str = list(our_str)

    ''' 
        Для каждой буквы строки проверяем есть ли уже в словаре
        Если есть - прибавляем цифру и добавляем в строку букву и количество 
        Если нет создаем э-т и добавляем в строку букву и количество 
    '''
    for let in array_str:
