...
def josephus_task(num_people, kill_num):
    index_kill = 0
    array_num_people = [i + 1 for i in range(num_people)]
    while len(array_num_people) > 1:
        index_kill += kill_num - 1
        if index_kill < len(array_num_people):
            array_num_people.pop(index_kill)
        else :
            while index_kill >= len(array_num_people):
                index_kill = index_kill - len(array_num_people)
            array_num_people.pop(index_kill)
    survivor = array_num_people[0]
    return survivor
